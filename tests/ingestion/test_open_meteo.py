...
def test_invalid_coordinates_raise_error(
    ...
    client = OpenMeteoClient(http_client=Mock())
...
def test_ingest_historical_persists_weather_hourly():
    client = Mock()
    storage = Mock()
    ...
    client.get_historical_weather.return_value = data
    storage.save_json.return_value = (
        "bronze/open_meteo/weather_hourly/test.json"
    )
    ...
    result = ingestion.ingest_historical(
        latitude=43.0,
        longitude=-2.5,
        start_date=date(2026, 8, 1),
        end_date=date(2026, 8, 1),
    ...
    client.get_historical_weather.assert_called_once()
